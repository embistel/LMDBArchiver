...
LMDB는 `MDB_NOSUBDIR`로 열립니다. 키는 UTF-8 `entry:<normalized/path>`이며 값은 `QDataStream::Qt_6_0`으로 기록한 다음 필드입니다.
...
6. Qt 파일 권한 비트
7. 압축 여부
8. 원본 또는 `qCompress(level=7)` 페이로드

압축 결과가 원본보다 작을 때만 압축 페이로드를 사용합니다. 쓰기 작업은 단일 LMDB 트랜잭션으로 커밋되며 예상 입력 크기에 따라 map size를 2배 단위로 확장합니다.
